            ...
            <div class="ora-storelist">
                <table id="store_table">
                    ...
                    foreach ($arStore as $val) {
                        ...
                        ?>
                        <tr class="store_row <?= $checked ?>" id="row_<?= $val["ID"] ?>"
                            onclick="setChangeStore(<?= $val["ID"] ?>);">
                            <?
                            if ($showImages) {
                                ?>
                                <td class="image_cell">
                                    <div class="image">
                                        ...
                                        <?
                                            ...
                                            ?>
                                            <img
                                                src="/bitrix/components/bitrix/sale.order.ajax/templates/visual/images/no_store.png"/>
                                        <?
                                        endif;
                                        ?>
                                    </div>
                            ...
                            <td class="<?= ($countCount != $i) ? "lilne" : "last" ?>">
                                <label for="store_<?= $val["ID"] ?>">
                                    ...
                                    <div class="phone"><?= GetMessage('MAP_PHONE'); ?>
                                        : <?= htmlspecialcharsbx($val["PHONE"]) ?></div>
                                    <div class="email"><?= GetMessage('MAP_EMAIL'); ?>: <a
                                            href="mailto:<?= htmlspecialcharsbx($val["EMAIL"]) ?>"><?= htmlspecialcharsbx($val["EMAIL"]) ?></a>
                                    </div>
                                    <div class="adres"><?= GetMessage('MAP_ADRES'); ?>
                                        : <?= htmlspecialcharsbx($val["ADDRESS"]) ?></div>
                                    <div class="shud"><?= GetMessage('MAP_WORK'); ?>
                                        : <?= htmlspecialcharsbx($val["SCHEDULE"]) ?></div>
                                </label>

                                <div class="desc"><?= GetMessage('MAP_DESC'); ?>
                                    : <?= htmlspecialcharsbx($val["DESCRIPTION"]) ?></div>
                            </td>
                        </tr>
                        <?
                        $i++;
